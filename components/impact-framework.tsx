          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white/80 backdrop-blur-sm px-4 py-2 rounded-lg shadow-sm border border-gray-200 inline-block mb-4">
              <p className="text-lg font-medium text-gray-700">
                <span className="text-emerald-600">IMPACT</span> = 1:1 adoption
              </p>
            </div>
            ...
            >
              ...
              <div className={`bg-white rounded-2xl p-6 shadow-lg border-2 transition-all duration-300 cursor-pointer h-full ${
                hoveredStep === `${step.letter}-${index}` 
                  ? 'border-emerald-400 shadow-2xl transform -translate-y-2 z-10' 
              ...
              {/* Hover Details Card */}
              {hoveredStep === `${step.letter}-${index}` && (
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-4 w-80 bg-white rounded-2xl shadow-2xl border-2 border-emerald-200 p-6 z-20 animate-in fade-in duration-200">
                  ...
                  <div className="space-y-2">
                    {step.programs.map((program, idx) => (
                      <div key={idx} className="bg-emerald-50 text-emerald-700 text-xs px-3 py-2 rounded-full inline-block mr-2 mb-2">
                        {program}
                      </div>
                    ))}
                  </div>
